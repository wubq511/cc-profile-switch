...
确认 profile 名称后，确定路径：
- macOS: `~/.cc-profile-switch/profiles/<name>/`
...
2. `<profile>/mcp.json`
3. `<profile>/claude-home/CLAUDE.md`
4. `<profile>/claude-home/settings.json`
5. `<profile>/claude-home/memory/auto/MEMRY.md`（可选，自动记忆）
...
| `mcp.json` 存在且有效 JSON | 读取文件，尝试 JSON.parse | 缺失 = ⚠️（可选），无效 JSON = ❌ |
...
| **mcpMode 合理** | `merge`（默认，项目 mcp.json 仍然加载）适合大多数场景。`strict` 仅在需要完全控制 MCP 时使用。`none` 表示不加载任何 MCP | 合理 = ✅，不合理 = ⚠️ |
...
读取 `<profile>/mcp.json`，逐项检查。

| 检查项 | 检查方法 | 判定 |
|---|---|---|
| **有效 JSON** | 尝试 JSON.parse | 有效 = ✅，无效 = ❌ |
...
| **使用变量而非硬编码** | 检查配置中是否使用 `${VAR}` 环境变量引用敏感信息（API key、token、路径），而非硬编码明文 | 使用变量 = ✅，有硬编码路径（可接受） = ⚠️，有硬编码密钥 = ❌ |
...
修复为正确的绝对路径：
- macOS: `~/.cc-profile-switch/profiles/<name>/claude-home/memory/auto`
...
```
并在 `settings.json` 的 `env` 中或系统环境变量中设置实际值。
