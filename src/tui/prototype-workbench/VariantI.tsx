// PROTOTYPE — Variant I: guidance "chosen mix" per the human's verdict (issue #29).
// One variant per guidance surface, picked from F/G/H interactively:
//   first-run     — G's dismissible welcome card (shows once per session)
//   empty states  — G's 2–3 line recipes with the concrete next step
//   errors        — H's boxed panel with numbered recovery steps
//   destructive   — G's inline panel: consequence + graduated options
//   shortcut help — F's full sheet INCLUDING the concepts section
//   discovery     — G's small just-in-time tips (search tip, MCP nudge)
// Overall density — G's middle: retiring contextual hints (a hint disappears
// after you use its key three times); steady state ends up compact.
...
export default function VariantI({ profiles }: VariantProps) {
  ...
  const searchBox = (
    <Box flexDirection="column">
      ...
      {searching && searchTipLive && !q ? (
        <Text dimColor>tip: searches everything</Text>
      ) : null}
    ...
    if (selRow.type === 'profile') {
      ...
      return (
        <Box flexDirection="column" flexGrow={1}>
          ...
          <Box flexGrow={1} />
          <Text color="cyan">{hintLine(['l', 'e', 'b', 'x', '/', '?'])}</Text>
        </Box>
      );
    }
    ...
    if (selRow.type === 'category') {
      ...
      return (
        <Box flexDirection="column" flexGrow={1}>
          ...
          <Box flexGrow={1} />
          <Text color="cyan">{hintLine(items.length ? ['e', 'a', '?'] : ['a', 'n', '?'])}</Text>
        </Box>
      );
    }
    ...
    return (
      <Box flexDirection="column" flexGrow={1}>
        ...
        {c === 'Plugins' ? (
          ...
          <>
            {skill?.kind === 'linked' ? (
              <Text dimColor>linked — removing it never deletes the source</Text>
            ) : null}
            <Text color="cyan">{hintLine(['e', 'x', '?'])}</Text>
          </>
        )}
      </Box>
    );
  ...
  if (mode === 'help') {
    return (
      <Box flexDirection="column" paddingX={2}>
        <Text bold>Profile Workbench · keys & concepts</Text>
        <Text> </Text>
        ...
        <Text> </Text>
        <Text dimColor> hints fade after you use a key {RETIRE_AFTER} times — this sheet is always here.</Text>
    ...
    <Box flexDirection="column" flexGrow={1}>
      <Box flexGrow={1} paddingX={1} gap={1}>
        <Box flexDirection="column" width={sideW} borderStyle="single" borderColor="gray" paddingX={1}>
          ...
          <Box flexGrow={1} />
          <Text dimColor>hints fade · [?] all keys</Text>
        </Box>
