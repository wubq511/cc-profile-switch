...
// the CLI harness (tsx test/perf/harness.ts) directly. Wall-clock fixture
// materialization time is deliberately NOT asserted here — §15.4 excludes
// CI from that gate.
//
// Materializing a tier writes thousands of small files and is extremely slow
// on hosted Windows runners, so each tier is materialized exactly once in
// beforeAll and shared read-only across every test. All async tests get a
// generous timeout so runner I/O noise cannot flake them.
...
import { buildFixturePlan, materializeFixture } from '../fixtures/generator';

// ---------------------------------------------------------------------------
// Shared materialized fixtures
// ---------------------------------------------------------------------------

// Generous per-test/hook timeout: hosted CI runners (Windows especially) are
// an order of magnitude slower than real machines for small-file I/O.
const PERF_TEST_TIMEOUT_MS = 120_000;

let baselineFixtureDir: string;
let threeXFixtureDir: string;

beforeAll(async () => {
  baselineFixtureDir = await fs.mkdtemp(path.join(tmpdir(), 'ccps-perf-baseline-'));
  await materializeFixture(
    buildFixturePlan({ tier: 'baseline', pathologies: [] }),
    baselineFixtureDir,
  );

  threeXFixtureDir = await fs.mkdtemp(path.join(tmpdir(), 'ccps-perf-3x-'));
  await materializeFixture(buildFixturePlan({ tier: '3x', pathologies: [] }), threeXFixtureDir);
...
describe('data-layer performance — baseline tier (issue #79)', () => {
  ...
  it(
    'loadWorkbenchData completes within threshold',
    async () => {
      ...
      const paths = getAppHomePaths(baselineFixtureDir);
  ...
  it(
    'buildSidebarRows completes within keystroke-repaint threshold',
    async () => {
      ...
      const data = await loadWorkbenchData(baselineFixtureDir);
  ...
  it(
    'buildSidebarRows with search query completes within threshold',
    async () => {
      const { loadWorkbenchData } = await import('../../src/tui/workbench/profile-data');
      const { buildSidebarRows } = await import('../../src/tui/workbench/sidebar-tree');
      const { CATEGORIES } = await import('../../src/tui/workbench/categories');
  ...
  it(
    'searchAllResources completes within threshold',
    async () => {
...
describe('3× tier loading-state rule (issue #79)', () => {
  ...
  it(
    'loadWorkbenchData completes within 2 s × CI multiplier',
    async () => {
      const { listProfilesForDisplay } = await import('../../src/core/profile-management');
      const { validateProfile } = await import('../../src/core/validator');
      const { getAppHomePaths } = await import('../../src/core/app-config');
      ...
      const start = performance.now();
      const paths = getAppHomePaths(threeXFixtureDir);
      const summaries = await listProfilesForDisplay({ appHomePath: paths.appHomePath });
      await Promise.all(
        summaries.map(async (summary) => {
          const claudeHome = path.join(paths.profilesPath, summary.name, 'claude-home');
  ...
  it(
    'buildSidebarRows completes within 2 s × CI multiplier',
    async () => {
      const { listProfilesForDisplay } = await import('../../src/core/profile-management');
      const { getAppHomePaths } = await import('../../src/core/app-config');
      const { buildSidebarRows } = await import('../../src/tui/workbench/sidebar-tree');
  ...
  it(
    'search filtering completes within 2 s × CI multiplier',
    async () => {
      const { listProfilesForDisplay } = await import('../../src/core/profile-management');
      const { getAppHomePaths } = await import('../../src/core/app-config');
      const { buildSidebarRows } = await import('../../src/tui/workbench/sidebar-tree');
...
describe('perf harness infrastructure (issue #79)', () => {
  it(
    'runHarness returns valid result structure',
    async () => {
      const { runHarness } = await import('./harness');
      // The mini tier is tiny, so let the harness materialize it end-to-end.
  ...
  it(
    'runHarness baseline tier returns all four measurements',
    async () => {
      const { runHarness } = await import('./harness');
      const result = await runHarness({
        tier: 'baseline',
        iterations: 2,
        fixtureDir: baselineFixtureDir,
      });
  ...
  it(
    'runHarness 3× tier returns all four measurements',
    async () => {
      const { runHarness } = await import('./harness');
      const result = await runHarness({
        tier: '3x',
        iterations: 2,
        fixtureDir: threeXFixtureDir,
      });
